    ...
    if (from_order > to_order) {  // high-to-low relation
      if (stmt->is_size()) {
        if (from_type == mesh::MeshElementType::Cell &&
          ...
          stmt->replace_with(Stmt::make<ConstStmt>(TypedConstant{12}));
        } else if (from_type == mesh::MeshElementType::Cell &&
                   stmt->to_type == mesh::MeshElementType::Face) {
          stmt->replace_with(Stmt::make<ConstStmt>(TypedConstant{6}));
        } else {
          stmt->replace_with(
              Stmt::make<ConstStmt>(TypedConstant{from_order + 1}));
        }
        ...
        if (from_type == mesh::MeshElementType::Cell &&
            stmt->to_type == mesh::MeshElementType::Edge) {
          to_size = block.push_back<ConstStmt>(TypedConstant{12});
        } else if (from_type == mesh::MeshElementType::Cell &&
                   stmt->to_type == mesh::MeshElementType::Face) {
          to_size = block.push_back<ConstStmt>(TypedConstant{6});
        } else {
